...
appname = "googlecast" #linkedin
Reviewfile_name =  appname +"_reviews.csv"
cleanReviewfile_name = appname+"_reviews_cols_clean.csv"

Tweetfile_name = appname+ "_tweets.csv"
cleanTweetfile_name = appname + "_tweets_cols_clean.csv"
...
def plotGraphs(tweetDict, reviewDict):
	tweetDict.groupby(["Date"]).count()['tweet'].plot.bar(stacked=True); #hist(bins=10)
	#plt.show()
...
def readTwitterBugsFeature(tweetDict):
	...
	#for features
	features = pd.read_csv(TweetFeature, header=None, usecols=[0,1]) #tweet and the classlabel for feature
	features = standardize_text(features, 0)
	print features[0], len(features.index)
	tweetDict.loc[tweetDict['tweet'].isin(features[0]), 'Class'] = 2 #bug:1, review:2, rest:0  
	print tweetDict.groupby(['Class']).count()
...
def readReviewBugsFeature(reviewDict):
	...
	#for features
	features = pd.read_csv(ReviewFeature, header=None, usecols=[0,1]) #tweet and the classlabel for feature
	features = standardize_text(features, 0)
	print features[0], len(features.index)
	reviewDict.loc[reviewDict['body'].isin(features[0]), 'Class'] = 2 #bug:1, review:2, rest:0  
	print reviewDict.groupby(['Class']).count()
